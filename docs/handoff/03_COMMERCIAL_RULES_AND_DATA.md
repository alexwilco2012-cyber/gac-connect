...
- **Launches and taxis — Crew change › Transfers** (17 Aug review + owner follow-up): launches live inside Crew change, not on a tab of their own — `/app/crew-change?section=transfers` (`/app/launches` redirects there). Crew-boat / workboat runs to vessels at anchor, grouped by port — Aberdeen and Macduff are the examples — showing each launch's **maximum capacity** and **whether freight is included** in the run; two fictional **taxi** operators (category `Taxis`, airport meet-and-greet, flight-tracked pickups); and a **flight-timed transport planner**: given the crew member's flight number the platform tracks the flight and times the taxi and the launch to it (on-signers: land → taxi → quay → launch; off-signers: launch → quay → taxi → check-in deadline), and a delay re-times every leg. The feed is **simulated** in this proof of concept and says so (production: a live flight-status feed, Skyscanner-style or the airport's own API). Launch and taxi operators are ordinary SVS-vetted marketplace suppliers (`Supplier.launch` / `Supplier.facts`) and appear in the marketplace and the SVS too. Rules in `src/lib/launches.ts` and `src/lib/transfers.ts` (unit-tested).
- **Procurement via Compass** (17 Aug review + owner follow-up): the client's list goes by email straight to **Compass, GAC's procurement branch**; Compass supplies what it can; where it cannot assist on a line it sources it through its network (a ship chandler), **pays the chandler itself**, and the client is **invoiced via Compass, under GAC** — one invoice, no third-party paperwork. **Nothing about a mark-up is ever shown to the client**: the invoice carries Compass's line prices and one total, and the words mark-up / margin do not appear on any client-facing screen (unit- and e2e-tested). `/app/procurement` is a working example: composed email shown on screen (illustrative `.example` address), Compass side simulated with labelled buttons. Rules in `src/lib/procurement.ts` (unit-tested).
- **Crew change** (17 Aug review): `/app/crew-change` gathers hotels (the two Hotels suppliers), transfers (taxis, launches, flight-timed transport — above), immigration guidance, and two templates the client fills in — an **Immigration Support Letter / LOI** per visa-national on-signer, which GAC endorses as agents and returns (GAC does not issue OKTB letters), and a **disembarkation / repatriation letter** per off-signer, which GAC prepares and sends to **UK Border Force** for endorsement, then returns. One letter per crew member. GAC informs, it does not advise. Templates carry obviously fictional demo values and a "do not enter real passport data" notice; stage machines in `src/lib/crewChange.ts` (unit-tested).
