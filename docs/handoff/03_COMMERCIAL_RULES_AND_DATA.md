...
- **Launches** (17 Aug review): a Launches tab (`/app/launches`) for crew-boat / workboat runs to vessels at anchor, grouped by port — Aberdeen and Macduff are the examples — showing each launch's **maximum capacity** and **whether freight is included** in the run. Launch operators are ordinary SVS-vetted marketplace suppliers (category `Launches`, `Supplier.launch` details) and appear in the marketplace and the SVS too. Rules in `src/lib/launches.ts` (unit-tested).
- **Procurement via Compass** (17 Aug review): the client's list goes by email straight to **Compass, GAC's procurement branch**; Compass supplies what it can; where it cannot assist on a line it sources it from a third party (a ship chandler), **pays the chandler itself**, and the client is **invoiced via Compass, under GAC, with a mark-up** — one invoice, no third-party paperwork. `/app/procurement` is a working example: composed email shown on screen (illustrative `.example` address), Compass side simulated with labelled buttons, mark-up percentage illustrative and labelled (never presented as GAC's actual margin). Rules in `src/lib/procurement.ts` (unit-tested).
- **Crew change** (17 Aug review): `/app/crew-change` gathers hotels (the two Hotels suppliers), immigration guidance, and two templates the client fills in — an **Immigration Support Letter / LOI** per visa-national on-signer, which GAC endorses as agents and returns (GAC does not issue OKTB letters), and a **disembarkation / repatriation letter** per off-signer, which GAC prepares and sends to **UK Border Force** for endorsement, then returns. One letter per crew member. GAC informs, it does not advise. Templates carry obviously fictional demo values and a "do not enter real passport data" notice; stage machines in `src/lib/crewChange.ts` (unit-tested).
