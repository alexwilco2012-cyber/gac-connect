...
- **17 Aug review tabs**: Launches (`/app/launches`, capacity + freight-included per launch, ports as examples), Procurement via Compass (`/app/procurement`, email to Compass → Compass supplies or sources from a chandler and pays them → invoiced via Compass under GAC with an illustrative, labelled mark-up), Crew change (`/app/crew-change`, hotels + immigration + LOI/repat templates → GAC endorses / UKBF endorses → returned). Compass replies and endorsements are simulated and say so on screen. Never real passport data, controlled-document codes, or employee names.
